#!/usr/bin/env bash
# Usage: ./submit_snehurka_exp.sh experiments/2026_13_04_sponge_med/
#    or: ./submit_snehurka_exp.sh 2026_13_04_sponge_med
#
# If the experiment directory contains a .sh file, uses that as the sbatch script.
# Otherwise falls back to the root submit_snehurka.sh.
...
if [[ -n "$SUBMIT_SH" ]]; then
    ...
    SUBMIT_SH="$SCRIPT_DIR/submit_snehurka.sh"
