...
mutable struct Particle <: AbstractParticle
		...
		h0 = η * dr
                ...
                # initialization
		obj.T = T_bg
		obj.ρ = background_density(obj.x[2], ρ0, T_bg, g, R_mass)
		obj.P = background_pressure(obj.x[2],ρ0, T_bg, g, R_mass)
...
function accelerate!(p::Particle, dt::Float64, g::Float64, z_t::Float64, z_β::Float64, γ_r::Float64)
        p.v += 0.5 * dt * (p.Dv + gravity(g) + damping_structure(p.x[2], z_t, z_β, γ_r)) # this is a vector sum
        p.Dv = VEC0
end
...
function run_sim(global_params::Dict, sim_params)
	...
	function verlet_step!(sys)
		...
		apply!(sys, p -> find_pot_temp!(p, ρ0, T_bg, R_gas))
	...
	metadata_dict["module"] = "StaticHopkinsTotal" # add module metadata manually
