...
Sets grad_δA, grad_u, grad_w to the zero vector.
"""
...
function compute_ebc_gradients!(p::AbstractParticle, q::AbstractParticle, r::Float64)
	@inbounds if (p.type == FLUID) || (p.type == INFLOW_INCOMING)
		...
		p.grad_δA += (q.δA - p.δA) / p.ρ * ker * x_pq
...
"""
function apply_extrapolation!(sys::ParticleSystem, ρ0::Float64, T_bg::Float64, g::Float64, R_mass::Float64, γ::Float64)
	for p in sys.particles
		if ((p.type == INFLOW_GHOST) || (p.type == OUTFLOW_GHOST)) && p.best_match_id > 0 && p.best_match_id <= length(sys.particles)
			...
			p.δA = q.δA + SmoothedParticles.dot(dx, q.grad_δA)
			p.A =  background_entropy(p.x[2], ρ0, T_bg, g, R_mass, γ) + p.δA 
